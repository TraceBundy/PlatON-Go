...
RUN apk add --no-cache make gcc musl-dev linux-headers g++ llvm bash cmake git gmp-dev openssl-dev libc-dev go
#
#RUN git clone https://github.com/dfinity/bn.git
#RUN cd bn && make && make install
#FROM registry.cn-shenzhen.aliyuncs.com/platon-dev/platon:base as builder
ADD . /PlatON-Go
RUN export https_proxy='http://10.10.8.222:8123' 
