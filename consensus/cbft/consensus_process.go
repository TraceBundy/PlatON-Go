...
func (cbft *Cbft) OnPrepareBlock(id string, msg *protocols.PrepareBlock) *MsgHandleError {
	...
	if _, err := cbft.verifyConsensusMsg(msg); err != nil {
		...
		return &MsgHandleError{err: err, disconnect: true, verifySign: false}
	}
	...
	if err := cbft.evPool.AddPrepareBlock(msg); err != nil {
		if _, ok := err.(*evidence.DuplicatePrepareBlockEvidence); ok {
			...
			//dup sign need forward
			return &MsgHandleError{err: err, disconnect: false, verifySign: true}
		}
	}
...
// Perform security rule verification，store in blockTree, Whether to start synchronization
func (cbft *Cbft) OnPrepareVote(id string, msg *protocols.PrepareVote) *MsgHandleError {
...
func (cbft *Cbft) OnViewChange(id string, msg *protocols.ViewChange) *MsgHandleError {
