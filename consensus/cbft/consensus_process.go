...
func (cbft *Cbft) OnPrepareVote(id string, msg *protocols.PrepareVote) (err error) {
	...
	cbft.log.Debug("After add prepare vote", "number", msg.BlockNumber, "hash", msg.BHash(), "votes", cbft.state.PrepareVoteLenByIndex(msg.BlockIndex))
	...
	for i := 0; i < len(blocks); i++ {
		...
		if err := cbft.safetyRules.QCBlockRules(block, qc); err != nil {
			if err.NewView() {
				...
				blockTree, _ := cbft.blockTree.MarshalJSON()
				cbft.log.Debug("QC block rule fail", "status", string(blockTree))
				return err
			}
		}
